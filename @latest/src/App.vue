  ...
  <main>
    ...
    <div class="container">
      ...
      <div class="cards-container" >
        <div v-for="(note , index) in notes" :key="note.id"  :class="{toggleCard: note.openCard}" :id="note.id"  class="card" :style="{backgroundColor: note.backgroundColor}">
          <h3>{{note.Title}}</h3>
          ...
          <span class="date">{{note.date.toLocaleDateString("en-US")}}</span>
            ...
            <span @click="note.openCard =!note.openCard" >
              <button v-if="!note.openCard">View</button>
              <button  v-if="note.openCard">x</button>
            </span>
          </div>
